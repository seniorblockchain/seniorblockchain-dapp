    ...
    }) =>
    {
        ...
        addComponents(
            _.fromPairs(_.map(options.themes, (theme, themeName) => [
                ...
                ))), (value, key) => [[`--senior-${e(key)}`, value], [`--senior-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
            ])),
        );
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`senior.customProps.background.${colorScheme}`);
            const foreground = theme(`senior.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--senior-${e(key)}`, value], [`--senior-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--senior-${e(key)}`, value], [`--senior-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                },
            };
        });
            ...
            theme: {
                extend: {
                    ...
                    colors: _.fromPairs(_.flatten(_.map(_.keys(flattenColorPalette(normalizeTheme(options.themes.default))), (name) => [
                        [name, `rgba(var(--senior-${name}-rgb), <alpha-value>)`],
                        [`on-${name}`, `rgba(var(--senior-on-${name}-rgb), <alpha-value>)`],
                    ]))),
                },
                senior  : {
                    customProps: {
